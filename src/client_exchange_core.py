...
def password_hash(password: str) -> str:
    ...
    return _SCRYPT_PREFIX + b64e(salt) + "$" + b64e(derived)


def verify_password(password: str, encoded_hash: str) -> bool:
    try:
        body = (encoded_hash or "")
        if not body.startswith(_SCRYPT_PREFIX):
            return False
        salt_part, _, derived_part = body[len(_SCRYPT_PREFIX):].partition("$")
        salt = b64d(salt_part)
        expected = b64d(derived_part)
